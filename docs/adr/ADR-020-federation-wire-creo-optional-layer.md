...
- **S4 relay = universal floor（first-class、設計確定 2026-06-28 / 実装中）**: 〔council 2026-06-28 で旧「deferred」を撤回〕**tailnet 依存を外した**結果 off-overlay peer が常態 → relay が「誰でも繋がる」の本体（NAT の隅のレアケースではない）。hub=dumb forward / target World=outbound dial。endpoints は direct 候補のみで relay は **advertise しない別レイヤー**（dialer が direct 全滅で hub relay）。**hole-punch は採らない**（D1、per-pair state を避ける）。VP dialer と対で実装。

  **transport 機構の決定（A: reliable server-initiated stream）**〔2026-06-28 mito × Claude、多ターン精査〕: relay は target へ **reliable・flow-controlled な server→client 配送**を要する（delegation の nostos future を運ぶ）。club-unison 1.4.0 の既存 server→client push（`UnisonChannel::send_event` を target の persistent channel に流す = "Arch B"）は **意図的に best-effort**（channel.rs の recv demux が event を `try_send`・buffer 256・full で drop して Response path の HoL を防ぐ）→ relay payload が backpressure 下で取りこぼれ、edge reconcile(D5) の drop→再送 churn を生む。よって **dedicated QUIC stream（flow-control 内蔵 = reliable）= Arch A** を採る。Occam: 「best-effort を edge reconcile で肩代わり」より「substrate に欠けている reliable server-initiated stream を1つ足す」方が、正しく十分な最小構成。

  **club-unison 1.5.0 primitive（Occam-最小 = 対称2編集、新 transport 動詞ゼロ）**: 既に `UnisonConn::open_bi()`（server が stream を開く動詞）は存在し identity 送信で使用中。眠っている `ChannelDirection::ServerToClient`（型・KDL `from="server"` に既出、runtime で無視）を起こすだけ。① **server 側**: `ConnectionContext` に `Arc<dyn UnisonConn>` を保持し `ctx.open_server_stream(channel)` を生やす（中身 = 既存 `open_bi`）。② **client 側**: `client_accept_bi_loop` が `__identity` 以外の server-initiated stream を、宣言済み `from="server"` channel handler へ routing（identity と同じ「先頭 frame method で振り分け」の一般化）。**`get_connection_by_principal` は足さない** — hub が `wld_id → Arc<UnisonConn>` map を持つ application 責務（substrate に漏らさない）。詳細は club-unison `design/server-initiated-stream.md`。

  **relay composition（hub、上記 primitive の上）**: world connect→Register で wld_id 既知 → hub が **transient** な `wld_id → Arc<UnisonConn>` map に登録（disconnect で除去 = durable 0 = D1）。source が direct 全滅 → hub の `relay` channel に `{to: wld_id, opaque payload}` → hub が target conn を引き `ctx.open_server_stream` で **per-session reliable stream** を開き、source↔target を **dumb bidirectional bridge**（envelope の routing tag だけ見て中身は blind = D5）。target の `from="server"` inbound handler が自 world へ流す。

  **cross-repo 順序（Occam = 下から）**: ① club-unison 1.5.0（primitive + test、別 design doc）→ ② chronista-hub（relay channel = wld_id map + bridge）→ ③ VP（dialer = direct 試行→relay fallback / target inbound handler）。①が②③の土台。
