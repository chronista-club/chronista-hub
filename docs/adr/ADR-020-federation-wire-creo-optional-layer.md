...
- **S2 registry: wld_id + endpoints index（実装済 2026-06-28、protocol 0.2.0）**: `worlds.Register` が `wld_id`（location 独立 routing key）+ `endpoints`（direct 到達候補 `["[GUA]:port"]` 配列）を read → registry を **wld_id keyed** で upsert（rid `vp-world:{wld_id}`、 handle は display 属性）。`Discover` が wld_id + endpoints を返す。cross-transport（REST tree read）も wld_id-keyed。VP は #606(wld_id)/#607(endpoint) で **produce 済**、 hub は **consume index**。e2e PASS（wld_id/endpoints round-trip）。残: VP **dialer**（discover→direct 試行→relay fallback）が consume 側未了 → **§S6 で HEv2 設計確定（2026-07-07）**。
...
- **S6 dialer = Happy Eyeballs v2（設計確定 2026-07-07、実装未了・consume 側=VP、club-unison API 追加予定 ~1.6.0 / hub protocol 無改修）**: S2/S4 が二度「残: VP dialer」と置いた consume 側の頭を、**RFC 8305 Happy Eyeballs v2 の staggered race** として確定。ladder（D3 `direct→relay`）を**逐次フォールバックでなく 1 本の時間差レース**に畳む —— 「到達性が degrade するだけで別 feature ではない」（D3）を *逐次* でなく *並行* で満たす方が doctrine に忠実。hub は無改修（endpoints=順序付き opaque 候補 D2 + relay channel S4 を consume するだけ）= ladder 設計が正しかった徴。
  ...
  - 〔設計〕wire-level walkthrough（stagger timeline / eager·lazy RTT 収支 / `connect → connect_race` API）: artifact `14155633-370c-4f4c-bdf7-f5f7b68c9e0f`。次 = **B**（`connect_race` skeleton を club-unison に起こす）。
