...
1. **endpoint liveness = reconciliation**: 庭師モデル（desired=registry × actual=reachability を heal）。presence/liveness は tail-loss 許容 tier（spine durability tier）。← fork でなく実装詳細（方向は doctrine 確定）。**§S6 の dialer win-cache（`wld_id→{勝者,RTT}`）が "actual reachability" を副産物で供給**（dial の勝敗が liveness シグナル）。
...
- **S2 registry: wld_id + endpoints index（実装済 2026-06-28、protocol 0.2.0）**: `worlds.Register` が `wld_id`（location 独立 routing key）+ `endpoints`（direct 到達候補 `["[GUA]:port"]` 配列）を read → registry を **wld_id keyed** で upsert（rid `vp-world:{wld_id}`、 handle は display 属性）。`Discover` が wld_id + endpoints を返す。cross-transport（REST tree read）も wld_id-keyed。VP は #606(wld_id)/#607(endpoint) で **produce 済**、 hub は **consume index**。e2e PASS（wld_id/endpoints round-trip）。残: VP **dialer**（discover→direct 試行→relay fallback）が consume 側未了 → **§S6 で HEv2 化・実装済（2026-07-11、club-unison 1.6.0 + VP #715）**。
...
- **S6 dialer = Happy Eyeballs v2（設計確定 2026-07-07 → 実装済 2026-07-11、club-unison 1.6.0 + VP #715、hub protocol 無改修）**: S2/S4 が二度「残: VP dialer」と置いた consume 側の頭を、**RFC 8305 Happy Eyeballs v2 の staggered race** として確定。ladder（D3 `direct→relay`）を**逐次フォールバックでなく 1 本の時間差レース**に畳む —— 「到達性が degrade するだけで別 feature ではない」（D3）を *逐次* でなく *並行* で満たす方が doctrine に忠実。hub は無改修（endpoints=順序付き opaque 候補 D2 + relay channel S4 を consume するだけ）= ladder 設計が正しかった徴。
  - **race 化**: `connect(url)`（単一 addr・その場 Endpoint 生成・単発、`quic.rs:378`）を `connect_race(candidates, cfg)` に。1 個の client Endpoint から順序付き候補を **stagger 250ms**（QUIC 1-RTT 基準で RFC 下限 100–150ms まで詰め可）で並行 arm、**最初に QUIC handshake 完了した候補が勝ち・残りは cancel**。「全滅」を誰も判定しない（唯一の遅延ノブ = 有界な stagger、per-endpoint timeout の「短すぎ=良経路誤棄／長すぎ=数秒待ち」ジレンマを構造的に消す）。
  - **relay = 最劣後候補（eager arm、〔user: 2026-07-07 確定〕）**: relay を「一番遅く armed・一番負けやすい候補」に均し、floor を「armed だが待たれない」で表現。**eager** = t=0 で relay も並走・握手だけ先に済ませ direct budget 切れまで採用抑止 → direct 全滅でも failover **+0 RTT**。代償 = 捨てるかもの投機握手 1 回だが relay ctx は transient・durable 0（D1）ゆえ軽い。tailnet 撤廃で off-overlay が常態＝relay が本体（D3-b、線 63）だから eager が正しい。lazy（direct stagger 出尽くし後に arm、hub 無駄握手 0／failover +1 RTT）は `RaceCfg.relay_handicap` の opt-in に残す。
  - **候補ランキング = pure function**: hub の opaque 順序付き候補（D2）に *消費側で* 意味を与える。`rank(cands, my_addrs)` は副作用ゼロ（data/calc/action 分離）—— 同一 prefix（/64,/48）共有 GUA＝同一サイト → 先頭、IPv4 導入時は family interleave（v6,v4,…）、relay は末尾。ranking（calc）と racing（action）を分離するのが「beautiful structure」の芯。
  - **win-cache → Open point #1（庭師）供給**: dialer が `wld_id → {勝者候補, RTT}` を保持 → 次回はそれを t=0・ハンディ無しで先頭起動（定常状態＝無駄ゼロの単発 direct、race は cold-path 安全網として温存）。この win-cache は "actual reachability" シグナルそのもので、**Open point #1「endpoint liveness = reconciliation（庭師 desired×actual）」の actual をタダで供給**。dial 高速化が liveness テレメトリを副産物で吐く。
  - **境界（誇張しない）**: ① **relay の loser cancellation は特別扱い** — relay は「別アドレス」でなく hub `open_server_stream("relay")` + target outbound dial の *別到達機構* ゆえ、負け時に hub 上の half-open registry（`wld_id→ctx` transient、`unison_server.rs:198`）を確実に tear down（放置＝per-connection state 漏れ＝D1 の durable 0 を欠く）。② **S&F（D3-c）は同期 race 外** — race は `direct↔relay` の同期 2 段のみ、S&F はその下の逐次 fallback（home-World daemon、hub でなく）。③ **IPv4 は deferred（D3、線 62）** ゆえ v6/v4 interleave は潜在、今の実効候補は「複数 GUA + relay」（multi-homed / firewall 閉塞 GUA には既に効く）。④ **relay→direct 昇格（QUIC path migration）は dialer v2 に分離** — hole-punch 不採用（D1）ゆえ昇格は rendezvous 不要経路に限定。0-RTT は勝者候補のみ許可（複数同時発火の replay 回避）。
  - 〔設計〕wire-level walkthrough（stagger timeline / eager·lazy RTT 収支 / `connect → connect_race` API）: artifact `14155633-370c-4f4c-bdf7-f5f7b68c9e0f`。
  - 〔実装 landing 2026-07-11〕**club-unison 1.6.0**（#86: `network::dial` = network 非依存 race engine + `QuicClient/ProtocolClient::connect_race` direct adapter。engine は仮想時間 unit 10、adapter は実 QUIC e2e 2 で固定）→ crates.io publish。**VP #715**（`daemon/dialer.rs`: `federate_wire_send` = discover → direct race（deadline 1.5s 有界）→ relay floor、`VP_FEDERATION_DIRECT` kill-switch、e2e: dead-decoy 混在から live を掴み wire envelope 配送）→ nightly merged。**remote direct は現状 TheWorld が dev cert のため trust で fast-fail → relay floor に degrade（正しい徴）— mesh cert（VP PR-3 InternalMeshKeypair）到来で direct が勝ち始める**。残 item: win-cache（庭師供給）/ relay-in-race（`Transport{Direct|Relay}` 抽象の設計 round が前提 — relay は quinn Connection でなく hub 経由 one-way stream ゆえ透過的 adapter にならない）/ rank() の prefix ソート・family interleave / stagger 実測較正。
